...
function init()
    ...
    enemy_station = CpuShip():setTemplate("Exuari Carrier"):setFaction("Exuari"):orderStandGround()
    ...
    --Create 50 Asteroids
    placeRandomInLine(Asteroid, 50, -7500, -10000, -12500, 30000, 2000)
    placeRandomInLine(VisualAsteroid, 50, -7500, -10000, -12500, 30000, 2000)
	
    -- Create the defense for the station
    CpuShip():setTemplate("Exuari Guard"):setFaction("Exuari"):setPosition(-44000, -14000):orderDefendTarget(enemy_station)
    CpuShip():setTemplate("Exuari Sentinel"):setFaction("Exuari"):setPosition(-47000, -14000):orderDefendTarget(enemy_station)
    CpuShip():setTemplate("Exuari Warden"):setFaction("Exuari"):setPosition(-46000, -18000):orderDefendTarget(enemy_station)
	
	--Hangars.create(enemy_station, "Exuari Interceptor", 3)
    ...
    exuari_RT4_guard1 = CpuShip():setTemplate("Exuari Fighter"):setFaction("Exuari"):setPosition(3550, 31250):setRotation(0)
    exuari_RT4_guard2 = CpuShip():setTemplate("Exuari Fighter"):setFaction("Exuari"):setPosition(3950, 31250):setRotation(180)
...
And destroy those Exuari scum while you are at it!]])
    ...
    if not exuari_RT4_guard1:isValid() and not exuari_RT4_guard2:isValid() then
        -- Not sure how you did it, but you managed to destroy the two Exuari's before they detroy RT4...
...
function missionWaitForAmbush(delta)
    if distance(player, main_station) < 50000 then
        ...
        ambush_main = CpuShip():setFaction("Exuari"):setTemplate("Exuari Alpha Striker"):setScanned(true):setPosition(x - 1667, y + 2611):setRotation(-80):orderAttack(player)
        ambush_side1 = CpuShip():setFaction("Exuari"):setTemplate("Exuari Beta Striker"):setScanned(true):setPosition(x - 736, y + 2875):setRotation(-80):orderAttack(player)
        ambush_side2 = CpuShip():setFaction("Exuari"):setTemplate("Exuari Gamma Striker"):setScanned(true):setPosition(x - 2542, y + 2208):setRotation(-80):orderAttack(player)
...
function missionTransportWaitForRecovery(delta)
    ...
    if mission_timer < 0 then
        ...
        CpuShip():setTemplate("Exuari Heavy Bomber"):setFaction("Exuari"):setPosition(x, y):orderAttack(player)
    end
...
Lead the assault on the Exuari base in sector E2, expect heavy resistance.]])
        ...
        CpuShip():setTemplate("Exuari Guard"):setFaction("Exuari"):setPosition(-44000, -14000):orderDefendTarget(enemy_station)
        CpuShip():setTemplate("Exuari Warden"):setFaction("Exuari"):setPosition(-47000, -14000):orderDefendTarget(enemy_station)
        CpuShip():setTemplate("Exuari Sentinel"):setFaction("Exuari"):setPosition(-44500, -15000):orderDefendTarget(enemy_station)
        CpuShip():setTemplate("Exuari Savior"):setFaction("Exuari"):setPosition(-43000, -9000):orderAttack(player)
        mission_state = nil
    end
end
