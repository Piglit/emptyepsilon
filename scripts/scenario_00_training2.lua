...
---
--- Your ship is a Orca missile cruiser - a jump-driven cruiser with broadside missiles, but without beam weapons.
---
--- Advice:
...
function init()
	...
	player = PlayerSpaceship():setTemplate("Orca M5P"):setCallSign("Rookie 1"):setFaction("Human Navy"):setPosition(pos0x, pos0y):setHeading(90)
...
function commsInstr()
	if not instr1 and timer > 8.0 then
		instr1 = true
		command:sendCommsMessage(player, "Here is Commander Saberhagen. In this training mission you will practice more advanced tactics using an Orca missile cruiser. You will need to find and destroy the ships of a scattered enemy convoy, and use your jump drive and your missiles to take 'em by surprise. Notice that your ship does not have any beam weapons. I will keep you updated about the positions of the next enemy targets on a public broadcast channel; you can look at it on your comms log if you missed one. Commander Saberhagen out.")
	end
